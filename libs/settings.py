...
DEFAULT_SETTINGS = {
    'main_window': {'show_camera_realtime': False,
                    'show_serial': False,
                    'keyboard_control': False,
                    'tab': 'python',
                    'must': {'fps': '', 'com_port': '', 'com_port_name': '', 'camera_id': '', 'camera_name': ''},
                    'option': {'window_size_width': 948,
                               'window_size_height': 796,
                               'window_showMaximized': False,
                               'show_serial': True}
                    },
    'line': {'token_1': 'ここにLINE通知用のtokenを貼付'},
    'command': {'py_command': 'A連打', 'mcu_command': 'A連打'},
    'key_config': {'keyboard': {  # キーボード操作については実装未定
        'button': {'Y': 'K_y',
                   'B': 'K_b',
                   'A': 'K_a',
                   'X': 'K_x',
        ...
        'joystick': {  # デフォルト値はSwitch Pro Controllerに最適化
            'direction': {'LStick': True, 'RStick': True},
            'button': {'Y': {'state': True, 'assign': 'button.3'},
            ...
            'hat': {'TOP': {'state': True, 'assign': 'button.11'},
